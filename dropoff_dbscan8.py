...
Taxi=pd.read_csv('dropofftaxi7.csv')

del(Taxi['Unnamed: 0'])


Taxi1=Taxi.loc[((Taxi['dropoff_cluster']==1582)),:]
Taxi2=Taxi.loc[~(Taxi['dropoff_cluster']==1582),:]
Taxi1['dropoff_cluster8']=Taxi1['dropoff_cluster']
...
rep_points['dropoff_cluster']=np.arange(0,830)
Taxi1['dropoff_cluster']=cluster_labels
Taxi1 = pd.merge(Taxi1,rep_points ,on=['dropoff_cluster'], how='left')

Taxi1['dropoff_cluster']=Taxi1['dropoff_cluster']+445+531+210+142+137+117

"""Taxi['route_index']= list(zip(Taxi['dropoff_cluster'],Taxi['pickup_cluster']))"""
Taxi1.to_csv('dropofftaxi8.csv')
Taxi2.to_csv('Not_dropofftaxi8.csv')